...
That last result is worth stating as a rule, because it is cheap to measure and it saves a conversion. **The Metal path resides the weights in fp32, so peak memory runs about 4× the int8 file** — measured at 3.98× on the 1.2B (1364 MB file → 5432 MB peak; its CPU run is 1.34×). At 2.80 GB the 2.7B projects to ≈ 11 GB against 11.7 GB of phone RAM, which is exactly what happens. The tempting fix — cut the prefill ladder — does nothing here: going from 12 signatures to 6 changed neither the failure nor its timing, because the weight term, not the variant term, is what overflows. Measure the ratio on a sibling that fits before you re-export anything: on a 12 GB phone it caps Metal-runnable bundles at roughly 2.4 GB.
