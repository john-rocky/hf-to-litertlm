...
- **The GPU executor's activation dtype must be decided per file, by measurement.** With the default fp16 activations the int8 file's thinking on the rhyme gate question ran 2021 tokens without closing `</think>` on Metal (CPU: `blue`); declaring `prefer_activation_type = "fp32"` (`scripts/set_activation_type.py`, weights untouched) closes it in 452 tokens and the file passes 8/8. The int4 file is the other way round: at fp16 it passes 8/8 and finishes 22/30 thinking-ON GSM8K questions inside 3584 tokens; the same weights with fp32 declared think ~1.7× longer and finish 0/30. So: **int8 ships with fp32 activations declared, int4 with the fp16 default** — and neither rule generalises without a thinking-mode run on the actual file.
