  ...
  Variant.fromJson(Map<String, dynamic> j)
      : file = j['file'] as String,
        ...
        recommended = ((j['recommended'] as List?) ?? const [])
            ...
            .toList(),
        // `.toList()` after `.cast<String>()` on purpose: the cast alone is lazy,
        // so a non-string element would surface at first use (inside resolve())
        // instead of failing here, at parse.
        platformNotes = (((j['requirements'] as Map<String, dynamic>?)?['platform_notes'])
                    as List?)
                ?.cast<String>().toList() ??
            const [],
        knownIssues = (j['known_issues'] as List?)?.cast<String>().toList() ?? const [],
        raw = j;
