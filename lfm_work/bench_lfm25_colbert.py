...
OUT_DIR = sys.argv[1] if len(sys.argv) > 1 else "out_lfm25_colbert_350m"
SEQS = (32, 128, 256, 512)
...
def bench(path, S, iters=20):
    ...
    ids = np.random.randint(10, 64401, (1, S)).astype(np.int32)
    ids[0, 0] = 1
...
def main():
    ...
    for kind in ("wi8fc", "fp16", "fp32"):
        path = os.path.join(OUT_DIR, f"colbert_{kind}.tflite")
        if not os.path.exists(path):
            continue
        out[kind] = {}
        for S in SEQS:
            ms = bench(path, S)
            out[kind][f"encode_{S}"] = round(ms, 1)
            print(f"{kind} encode_{S}: {ms:.1f} ms  ({S/ms*1000:.0f} tok/s)",
                  flush=True)
