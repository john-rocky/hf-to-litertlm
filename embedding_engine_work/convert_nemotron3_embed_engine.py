#!/usr/bin/env python3
"""nvidia/Nemotron-3-Embed-1B-BF16 -> the LiteRT-LM 0.17.0 EmbeddingEngine bundle contract.

    .venv-092/bin/python scripts/convert_nemotron3_embed_engine.py <model_dir_or_id> out/nemotron3_embed_1b_engine

Same split as convert_granite_embedding_r2_engine.py (contract in that docstring):
  TF_LITE_EMBEDDER      `token` int32[1] -> raw `embed_tokens` row f32[1,1,2048]
  TF_LITE_TEXT_ENCODER  `encoder_<S>`: `embeddings` f32[1,S,2048] + `input_mask` f32[1,S]
                        -> mean over valid positions (prompt included, as sentence-transformers
                        does with include_prompt=True), L2-normalized f32[1,2048]
The bidirectional additive bias is built from `input_mask` exactly as convert_nemotron3_embed.py
builds it from attention_mask (config.is_causal=false model, no GATHER_ND). Stale pad rows in
the runtime's `embeddings` buffer are replaced with zeros via torch.where before the body, so a
non-finite stale row can never reach the mean (see convert_lfm25_embedding_engine.py).
"""
import argparse
import json
import os
...
from convert_nemotron3_embed import (  # noqa: E402
    DEFAULT_MODEL, PAD_ID, Embedder, load_model, sample, torch_ref,
)
...
class Encoder(nn.Module):
    ...
    def forward(self, embeddings, input_mask):
        ...
        h = self.body(inputs_embeds=emb, attention_mask=bias, use_cache=False).last_hidden_state
...
def main():
    ...
    ap.add_argument("out_dir", nargs="?", default="out/nemotron3_embed_1b_engine")
