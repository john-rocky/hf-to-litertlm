...
| tokenizer | `tokenizer.json`; the runtime does **not** run its post-processor, so a model whose CLS position expects `<bos>` declares it in `EmbeddingMetadata.bos_token` and the engine's `insert_special_tokens` (default true) inserts it; a model whose post-processor adds bos AND eos (Giga-Embeddings, `<s> A </s>`) declares both, and the engine prepends bos and appends eos (a TRUNCATE keeps the eos at the end of the window) |
...
| Giga-Embeddings-instruct-480M-0826 | `convert_giga_embed_engine.py` + `pack_giga_embed.sh` | mean (`<s>` + prompt + text + `</s>`) | `<s>` (id 1) as bos AND `</s>` (id 2) as eos | 509 MB |

Parity gate (two venvs, because the exporter pins litert-torch 0.9.2 and the engine needs litert-lm 0.17.0): `gate_tflite_embed_half.py` embeds a text list through the shipped `.tflite` with the HF tokenizer, `gate_embedding_engine.py` embeds the same list through the bundle, `compare_embedding_gate.py` prints per-text cosine. All four bundles: cosine 1.000000 against their `.tflite` on 10 texts, on a Mac (Python) and on a Galaxy S26 (Kotlin `EmbeddingEngine`, litertlm-android 0.17.0). One more trap from the Giga run: the Kotlin gate reads one text per line, so a prompt containing a newline (`Instruct: …\nQuery: …`) cannot reach the phone verbatim — derive a newline-replaced device set and recompute the host halves on it; the replacement moves vectors by ~1%, so it is not cosmetic.
