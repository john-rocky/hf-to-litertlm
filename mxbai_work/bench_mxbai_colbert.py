#!/usr/bin/env python3
"""Mac CPU/XNNPACK latency for the mxbai-edge-colbert LiteRT artifacts.

    python bench_mxbai_colbert.py <out_dir> [--variants wi8fc,fp16]

Serialized, one variant at a time — parallel runs contaminate the numbers.
Check the machine is quiet first (other sessions run evals on this Mac).
"""
import argparse
import os
import statistics
...
def main():
    ...
    ap.add_argument("--seqs", default="48,128,256,512")
    ...
    for v in args.variants.split(","):
        path = os.path.join(args.out_dir, f"colbert_{v}.tflite")
        ...
        for S in [int(s) for s in args.seqs.split(",")]:
            try:
                r = it.get_signature_runner(f"encode_{S}")
            except Exception:
                continue
            n = int(S * 0.75)
            x = np.full((1, S), PAD_ID, np.int32)
            m = np.zeros((1, S), np.int32)
            x[0, :n] = np.random.default_rng(0).integers(10, 50000, n)
            m[0, :n] = 1
            ...
            print(f"  encode_{S:<4d} {med:7.1f} ms  (min {min(ts):.1f})  "
