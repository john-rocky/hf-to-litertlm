...
DECO=out/smolvlm2-2.2b-decoder
VISO=out/smolvlm2-2.2b-vision
...
echo "### 1. vision (SigLIP 384/patch14 static-pos + pixel-shuffle x3 + connector -> 81x2048)"
IMG_SIZE=384 $PY scripts/convert_smolvlm2_vision.py "$SRC" "$VISO" 2>&1 | grep -vE "$G" | tail -2
cat "$VISO/result.json" | $PY -c "import sys,json;d=json.load(sys.stdin);print('vision:',{k:d.get(k) for k in ['ok','image_size','enc_out','adp_out','enc_ops','adp_ops','end2end_corr','error_head']})"
echo "### 2. extract SmolLM2-1.7B (Llama) decoder"
...
for name in ["vision_encoder","vision_adapter"]:
    o=f"$VISO/{name}_int8.tflite"
    if os.path.exists(o): os.remove(o)
    quantizer.Quantizer(f"$VISO/{name}.tflite", r.dynamic_wi8_afp32()).quantize().export_model(o)
    print(name,"int8",round(os.path.getsize(o)/1e6,1),"MB")
PYEOF
echo "### 6. build fast_vlm bundle (image 384)"
DEC="$DECO" VIS="$VISO" CACHE=2048 IMAGE_SIZE=384 OUT_NAME=SmolVLM2-2.2B.litertlm $PY scripts/build_smolvlm_bundle.py 2>&1 | tail -2
ls -la out/internvl-bundle/SmolVLM2-2.2B.litertlm | awk '{print "BUNDLE:", $5/1e9" GB"}'
echo "### DONE"
