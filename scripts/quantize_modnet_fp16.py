"""FLOAT16 weight quantization of the MODNet .tflite (AI Edge Quantizer,
FLOAT_CASTING) — GPU-native fp16, matches the MiDaS/U2Net/Zero-DCE convention.

    ~/clipconv/bin/python scripts/quantize_modnet_fp16.py <fp32.tflite> [fp16_out.tflite]
"""
import sys, os
import numpy as np
...
def main():
    fp32 = sys.argv[1] if len(sys.argv) > 1 else "out/modnet/modnet_512.tflite"
    out = sys.argv[2] if len(sys.argv) > 2 else "out/modnet/modnet_512_fp16.tflite"
    ...
    # fidelity: fp16 vs fp32 on a random [0,1] image
    rng = np.random.default_rng(0)
    hwc = rng.random((size, size, 3), dtype=np.float32)
