...
reproduce() {
  ...
  granite42-3b)        dl ibm-granite/granite-4.2-3b $SM/granite-4.2-3b
  ...
  minicpm5-2b)         RECIPES="int4 int8" OUT=out/$key bash minicpm_work/convert_minicpm5_2b.sh ;;  # shipped mlboydaisuke/MiniCPM5-2B-LiteRT (int8 ship = fp32 activations declared via scripts/set_activation_type.py); hybrid thinking; verbatim jinja (USE_JINJA) + thought channel; int4 needs the in-place zero-scale fix (13 dead layer-0 MLP rows); int8 = desktop/Android (2.33 GB main section); the int4 file is also litert-community/MiniCPM5-2B's int4
  minicpm5-2b-readme-int8) bash minicpm_work/export_minicpm5_2b_readme.sh dynamic_wi8_afp32 out/$key ;;  # litert-community/MiniCPM5-2B's int8 (2026-09-13): the LiteRT-LM models/minicpm5 README command as written, pbtext pinned to b5e34ab1; released litert-torch 0.9.4 (PY=<that venv>); in-graph int8 embedding + lm_head, one prefill signature (128), canonical template (thinking off unless the app turns it on)
