...
DECO=out/internvl-1b-decoder
VISO=out/internvl-1b-vision
GREP='Loading weights|it/s|Redirects|register_constant|FlashAttention|torch_dtype.*deprecated|KernelPreference|ScaleCalculation|FutureWarning|copyreg|InitializeLog|XNNPACK|arithmetic ops|incorrect regex|fix_mistral'

echo "### 1. extract Qwen2.5-0.5B decoder"
$PY scripts/prep_internvl_decoder.py "$SRC" "$LLM" 2>&1 | grep -vE "$GREP" | tail -4
...
DEC="$DECO" VIS="$VISO" TOK=sp OUT_NAME=InternVL3-1B.litertlm $PY scripts/build_internvl_bundle.py 2>&1 | tail -2
ls -la out/internvl-bundle/InternVL3-1B.litertlm | awk '{print "BUNDLE:", $NF, $5/1e9" GB"}'
echo "### DONE"
