#!/bin/bash
# Full InternVL3_5-2B -> fast_vlm .litertlm pipeline. Adapted from ship_internvl_1b.sh.
...
SRC=src_models/internvl3_5-2b
LLM=src_models/internvl3_5-2b-llm
DECO=out/internvl3_5-2b-decoder
VISO=out/internvl3_5-2b-vision
...
echo "### 0. download OpenGVLab/InternVL3_5-2B"
$PY -c "from huggingface_hub import snapshot_download; snapshot_download('OpenGVLab/InternVL3_5-2B', local_dir='$SRC', ignore_patterns=['*.pth','*.bin.index.json.bak'])" 2>&1 | grep -vE "$GREP" | tail -2
...
DEC="$DECO" VIS="$VISO" TOK=sp OUT_NAME=InternVL3_5-2B.litertlm $PY scripts/build_internvl_bundle.py 2>&1 | tail -2
ls -la out/internvl-bundle/InternVL3_5-2B.litertlm | awk '{print "BUNDLE:", $NF, $5/1e9" GB"}'
echo "### DONE"
