#!/usr/bin/env python3
"""Mac CPU/XNNPACK latency for the ettin-reranker LiteRT artifacts.

    python bench_ettin_reranker.py <out_dir> [--variants wi8fc,fp16]
...
def main():
    ...
    for v in args.variants.split(","):
        path = os.path.join(args.out_dir, f"reranker_{v}.tflite")
        ...
        for S in [int(s) for s in args.seqs.split(",")]:
            try:
                r = it.get_signature_runner(f"score_{S}")
            except Exception:
                continue
            n = int(S * 0.75)
            ...
            print(f"  score_{S:<4d} {med:7.1f} ms  (min {min(ts):.1f})  "
