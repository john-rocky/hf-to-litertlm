...
Per the MiniCPM5 conversion guide: export with --quantization_recipe="" first,
then quantize the tf_lite_prefill_decode tflite with a model-specific
ai-edge-quantizer recipe, then rebuild the .litertlm (metadata + HF tokenizer +
quantized tflite) with litert-lm-builder.

Modes:
  inspect <file.tflite|file.litertlm>   dump per-tensor quantization layout
  apply   <in.litertlm> <out.litertlm> [--recipe wi4b32_wi8|wi4b128_wi8|wi8|wi8fc]
"""
import argparse
import os
import shutil
...
# The lm_head FULLY_CONNECTED in the decode subgraph. The vocab matrix is ONE
# buffer shared by the embedder and the head, so this rule + the
# EMBEDDING_LOOKUP rule must land it at the SAME quantization (int8-cw),
# matching the official file: 163 FCs int4-b32 + one vocab tensor int8-cw.
#
# ⚠ If this regex misses, the `.*` int4 FC rule claims the head while the
# embedding rule still wants int8 — one buffer, two quantizations — and
# ai-edge-quantizer resolves that by DUPLICATING the vocab table, once per
# subgraph (look for `_duplicated_` in tensor names). On granite-4.0-h-1b with
# 11 prefill signatures + decode that was 12 x 154 MB = 1.85 GB, i.e. 68% of
# the file, making the "int4" bundle (2.79 GB) LARGER than its int8 sibling
# (1.57 GiB) while the int4 linears themselves were perfectly packed.
# Verified 2026-08-17: same float export, wi8fc (no conflict) keeps ONE
# 154.1 MB vocab buffer; wi4b32_wi8 with a missing regex made twelve.
#
# `decode_logits_output` is the minicpm5 output-tensor name; `Linear_lm_head`
# is the HF module path in the op scope and is what generalises across models
# (any causal LM whose head module is named `lm_head`). Keep both.
LM_HEAD_REGEX = "(decode_logits_output|Linear_lm_head)"
...
def build_recipe(kind, algo="minmax"):
    ...
    if kind == "wi8":
        ...
        # ALL_SUPPORTED int8 breaks the conv layers (no output); leave convs float.
    ...
    elif kind == "fp16fc_i8emb":
        rm.add_weight_only_config(
            regex=".*", operation_name=OP.FULLY_CONNECTED, num_bits=16,
    ...
    elif kind in ("fp16fc_i8emb_shared", "fp16fc_i8emb_shared_dynamic_head", "fp16fc_i8emb_dynhead"):
        rm.add_weight_only_config(
            regex=".*", operation_name=OP.FULLY_CONNECTED, num_bits=16,
            granularity=G.CHANNELWISE, algorithm_key=AlgorithmName.FLOAT_CASTING,
        )
        # lm_head_scope.json verifies decode (not the prompt's typo decide).
        head_config = (rm.add_weight_only_config if kind == "fp16fc_i8emb_shared"
                       else rm.add_dynamic_config)
        head_config(regex=r"^decode_logits_output;$",
                    operation_name=OP.FULLY_CONNECTED, num_bits=8,
                    granularity=G.CHANNELWISE)
        rm.add_dynamic_config(
            regex=".*", operation_name=OP.EMBEDDING_LOOKUP, num_bits=8,
            granularity=G.CHANNELWISE,
        ...
        rm.add_dynamic_config(regex=".*", operation_name=OP.FULLY_CONNECTED, num_bits=8)
    elif kind == "embed_only":
        rm.add_dynamic_config(regex=".*", operation_name=OP.EMBEDDING_LOOKUP,
                              num_bits=8, granularity=G.CHANNELWISE)
    elif kind == "fc_no_lmhead":
        rm.add_dynamic_config(regex=".*", operation_name=OP.FULLY_CONNECTED, num_bits=8)
        rm.add_dynamic_config(regex=".*", operation_name=OP.EMBEDDING_LOOKUP,
                              num_bits=8, granularity=G.CHANNELWISE)
        # Verified against the SAME float graph: decode subgraph FC op 2123,
        # output tensor 2555 named exactly decode_logits_output (vocab x 1024).
        rm.add_quantization_config(regex=r"^decode_logits_output;$",
                                   operation_name=OP.FULLY_CONNECTED,
                                   algorithm_key=AlgorithmName.NO_QUANTIZE)
    elif kind in ("wo_int8", "wfp16"):
        bits = 8 if kind == "wo_int8" else 16
        algorithm = AlgorithmName.MIN_MAX_UNIFORM_QUANT if bits == 8 else AlgorithmName.FLOAT_CASTING
        for op in (OP.FULLY_CONNECTED, OP.EMBEDDING_LOOKUP):
            rm.add_weight_only_config(regex=".*", operation_name=op, num_bits=bits,
                                      granularity=G.CHANNELWISE, algorithm_key=algorithm)
    elif kind in ("wi4b32_wi8", "wi4b128_wi8"):
        # Official card: "mixed INT4-block32(linear)/INT8(embed and lmhead)".
        # int4 blockwise for all FULLY_CONNECTED, then int8 overrides for the
        # embedder + the lm_head/logits FC (regex refined from official-tflite
        # inspection). Block size is model-dependent (b128 ships faster on 4B
        # reasoning models; b32 is the quality-conservative choice) — sweep both.
        blk = {"wi4b32_wi8": G.BLOCKWISE_32, "wi4b128_wi8": G.BLOCKWISE_128}[kind]
        rm.add_dynamic_config(
            regex=".*", operation_name=OP.FULLY_CONNECTED, num_bits=4,
            granularity=blk, algorithm_key=alg4,
        )
        rm.add_dynamic_config(
...
def apply_quant(in_litertlm, out_litertlm, kind, algo="minmax"):
    ...
    builder = (os.environ.get("LITERT_LM_BUILDER")
               or shutil.which("litert-lm-builder")
               or os.path.expanduser("~/venvs/minicpm5/bin/litert-lm-builder"))
    cmd = [
...
def main():
    ...
    pa.add_argument("--recipe", default="wi4b32_wi8",
                    choices=["wi4b32_wi8", "wi4b128_wi8", "wi8", "wi8fc", "fc_only", "embed_only", "fc_no_lmhead", "wo_int8", "wfp16", "fp16fc_i8emb", "fp16fc_i8emb_shared", "fp16fc_i8emb_shared_dynamic_head", "fp16fc_i8emb_dynhead"])
