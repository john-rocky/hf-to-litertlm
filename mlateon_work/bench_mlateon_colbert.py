#!/usr/bin/env python3
"""Mac CPU/XNNPACK latency for the mLateOn LiteRT artifacts.

    python bench_mlateon_colbert.py <out_dir> [--variants wi8fc,fp16]

Serialized, one variant at a time — parallel runs contaminate the numbers.
Bench on an otherwise idle machine.
"""
import argparse
import os
import statistics
...
def main():
    ...
    ap.add_argument("--seqs", default="32,128,256,512")
    ...
    for v in args.variants.split(","):
        ...
        for S in [int(s) for s in args.seqs.split(",")]:
            ...
            x[0, :n] = np.random.default_rng(0).integers(10, 255000, n)
            m[0, :n] = 1
