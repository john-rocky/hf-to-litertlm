...
- **Progress + cancel**, share via the system sheet, PNGs also land in `Android/data/com.bonsai.imagegen/files/outputs/` (adb-pullable).
...
D=/sdcard/Android/data/com.bonsai.imagegen/files
...
```bash
adb shell am start -n com.bonsai.imagegen/.MainActivity \
...
If this app moves into a shared samples repository: change `applicationId`/namespace to that repo's convention (e.g. `com.google.ai.edge.samples.bonsai`), adjust license-header attribution, and consider a Compose rewrite of `MainActivity` if the repo standardizes on Compose (the pipeline/tokenizer/math files are UI-independent).
