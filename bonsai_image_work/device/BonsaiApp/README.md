...
2. **Project**: `xcodegen generate` (project spec in `project.yml`), then open `BonsaiApp.xcodeproj` and set your signing team.
...
If this app moves into a shared samples repository: switch `PRODUCT_BUNDLE_IDENTIFIER` to that repo's convention (e.g. `com.google.ai.edge.Bonsai`) — note the current id deliberately reuses the local verification container so the model set survives reinstalls; adjust the license-header attribution to the repo's convention; and the LiteRT dependency (currently the prebuilt `CLiteRTLM.framework` via SPM) can be swapped for a from-source `CLiteRT.xcframework` provided it exports the classic `TfLiteInterpreter` C API and `TfLiteXNNPackDelegate*`.
