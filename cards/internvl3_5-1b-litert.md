...
# InternVL3.5-1B — LiteRT-LM (on-device Vision-Language Model)

[OpenGVLab/InternVL3_5-1B](https://huggingface.co/OpenGVLab/InternVL3_5-1B) converted to the
...
grounding. The Qwen3-0.6B decoder uses the same **blockwise-32 + OCTAV int4** recipe that scores
...
- LiteRT-LM `fast_vlm` bundle: VISION_ENCODER (`[1,448,448,3]`→`[1,256,4096]`) + VISION_ADAPTER
  (`[1,256,4096]`→`[1,256,1024]`, matched to the Qwen3-0.6B hidden size) + single-token EMBEDDER +
